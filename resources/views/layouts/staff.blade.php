...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <link rel="icon" type="image/jpeg" href="{{ asset('images/logo-usbypkp.jpg') }}">
        <link rel="apple-touch-icon" href="{{ asset('images/logo-usbypkp.jpg') }}">
    ...
    <body class="bg-gray-50 text-gray-800 antialiased" x-data="{ mobileMenuOpen: false }">
        @php
            $staffUnreadAnnouncementCount = \App\Models\Employee\Announcement::query()
                ->active()
                ->where(function ($query) {
                    $query
                        ...
                        ->orWhere('target_audience', 'employee')
        ...
        <header class="lg:hidden bg-white border-b border-gray-200 sticky top-0 z-40">
            <div class="flex items-center justify-between px-4 h-14">
                ...
                <div class="flex items-center gap-2">
                    <a
                        href="{{ route('staff.pengumuman.index') }}"
                        title="Notifikasi Pengumuman"
                        aria-label="Notifikasi Pengumuman"
                        class="relative w-9 h-9 rounded-full bg-blue-50 text-blue-700 border border-blue-100 flex items-center justify-center hover:bg-blue-100 transition-colors"
                    >
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                        </svg>

                        @if($staffUnreadAnnouncementCount > 0)
                            <span class="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold leading-4 text-center shadow">
                                {{ $staffUnreadAnnouncementCount > 9 ? '9+' : $staffUnreadAnnouncementCount }}
                            </span>
                        @endif
                    </a>
                </div>
